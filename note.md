...
### 人力代碼空白與預設值
- **步驟 1 機台表（Scheduling/index.vue）**：人力代碼欄位為空白時顯示「-」，不顯示預設文字。
- **自動排班**：人力代碼空白時，系統執行時預設為「手1」；建立 `machinesForSchedule` 時寫入 `人力代碼: m.人力代碼 || '手1'` 與 `isDefaultLabor: true`（僅當原本空白時）。
- **排班服務（schedulingService.js）**：品號層級寫入 `isDefaultLabor`（機台為空白預設手1 時為 true；從品號資料庫帶出時為 false），供前端區分「手1-空白預設值」與「原本就填手1」。
- **排班結果列表（ListView.vue）**：代碼顯示邏輯改為「僅當代碼為手1 且 `product.isDefaultLabor` 時顯示『手1-空白預設值』」，其餘顯示原代碼；品號選擇對話框內代碼顯示同步此規則。
- **人力代碼解析（laborCodeParser.js）**：新增「手1+1」代碼規則，解析為兩人一機台（共 2 人、1 機台），供自動排班演算法使用。

### 總覽多品號保留（OverviewView.vue）
- **選擇操作方式（移動/複製/交換）**：確認拖放後 POST 改送**新格式**（`sourceItem` / `targetItem`，含完整 `products` 陣列），不再使用 `convertNewToOld`。避免舊格式只取第一個品號導致寫入後多品號被覆蓋（例如機台 1 從兩品號變成單一品號）。
- **移除區拖放**：從品號拖到「拖放到此處移除人員」後 POST 亦改送新格式（`sourceItem`），同樣保留該機台多品號。
- 本地 `allScheduleData`（舊格式）仍同步更新供內部使用，僅後端 payload 改為新格式。
